...
def _decode_general_name(backend, gn):
    if gn.type == backend._lib.GEN_DNS:
        ...
        # asn1_string_to_utf8 here because it doesn't properly convert
        # utf8 from ia5strings.
        data = _asn1_string_to_bytes(backend, gn.d.uniformResourceIdentifier).decode(
            "utf8"
        )
    ...
    def parse(self, x509_obj):
        ...
        for i in range(self.ext_count(x509_obj)):
            ...
            oid = x509.ObjectIdentifier(
                _obj2txt(
                    self._backend, self._backend._lib.X509_EXTENSION_get_object(ext)
                )
            )
            if oid in seen_oids:
            ...
            try:
                ...
                if ext_data == self._backend._ffi.NULL:
                    ...
                    raise ValueError(
                        "The {} extension is invalid and can't be " "parsed".format(oid)
                    )
...
def _decode_dist_points(backend, cdps):
    ...
    for i in range(num):
        ...
        dist_points.append(
            x509.DistributionPoint(full_name, relative_name, reasons, crl_issuer)
        )
...
_OCSP_SINGLERESP_EXTENSION_HANDLERS_SCT = {
    ExtensionOID.SIGNED_CERTIFICATE_TIMESTAMPS: (_decode_signed_certificate_timestamps)
}
